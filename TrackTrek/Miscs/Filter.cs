...
namespace TrackTrek.Miscs
{
    ...
    {
        private static List<string> blacklistedVideoKeywords = new List<string> { "Official HD Video", "Official Video", "】", "(HD", "(Live", "(Deluxe", "( Video)", "( 4K Video)" }; // The program will ignore any video with these (Some live videos don't have the exact same sound as studio quality)
        private static List<string> deletedVideoKeywords = new List<string> { "HD", "lyrics", "Lyrics", "Official", "official", "(Clean Version)", "(Live)", "(Official Audio)", "(Remastered)", "(HQ)", "(  Video)", "( Audio)", "(Audio)", "[HQ]", "[Official Music Video]", "[Extended]", "[LYRICS]", "(Official Video)", "(official video)" }; // The program will acccept videos with these keywords, but will delete these keywords in the title
