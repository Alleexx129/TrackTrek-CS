    ...
    internal class Searching
    {
        ...
        public static async Task<VideoInfo> FetchAndUpdateWithItune(VideoInfo videoInfo) // All infos are filtered, requires title and artist (using for yt searching)
        {
            ...
            foreach (JsonObject item in responseJson)
            {
                ...
                string currentArtist = Filter.FilterArtist(item["artistName"].ToString().ToCapitalFirst());
                string currentAlbum = Filter.FilterAlbum(item["collectionName"].ToString());

                string[] titleAndArtist = Filter.ToTitleAndArtist(videoInfo.Title.ToCapitalFirst(), videoInfo.GetArtist().ToCapitalFirst());
                ...
                {
                    currentTitleConfidence = Fuzz.Ratio(currentTitle.ToString(), titleAndArtist[0]);
                    currentAuthorConfidence = Fuzz.Ratio(currentArtist.ToString(), titleAndArtist[1]);
                    titleAndArtist = [titleAndArtist[1].ToString(), titleAndArtist[0].ToString()];
                }

                Sys.debug($"{currentAuthorConfidence} = {currentArtist.ToString()} + {titleAndArtist[1]}");
                Sys.debug($"{currentTitleConfidence} = {currentTitle.ToString()} + {titleAndArtist[0]}");
