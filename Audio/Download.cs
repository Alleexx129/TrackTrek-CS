    ...
    internal class Download
    {
        ...
        */
        ...
        public static async Task<VideoInfo> DownloadAudio(string link) // done
        {
            ...
            {
                filepath = Path.Combine(AppContext.BaseDirectory, "tools", "windows");
                filename = Path.Combine(filepath, "yt-dlp.exe");
            } else if (OperatingSystem.IsLinux())
            {
                filepath = Path.Combine(AppContext.BaseDirectory, "tools", "linux");
                filename = Path.Combine(filepath, "yt-dlp_linux");
            }
            ...
            var processStartInfo = new ProcessStartInfo
            {
                ...
                Arguments = $"""-f bestaudio --ffmpeg-location "{filepath}" --print "Title: %(title|Unknown)s\nArtist: %(uploader|Unknown)s\nThumbnail: %(thumbnail|Unknown)s" --encoding utf-8 --replace-in-metadata title "[ ]+" " " --no-simulate --replace-in-metadata "uploader" "^\\s+" "" -P "{AppSettings.DownloadFolder}" -o "%(title)s - %(uploader)s.%(ext)s" -t mp3 "{link}" """,
                RedirectStandardOutput = true,
            ...
            using (var process = new Process { StartInfo = processStartInfo })
            {
                ...
                videoInfo.Title = Filter.FilterTitle(title);
                videoInfo.YoutubeArtist = Filter.FilterArtist(author);
                videoInfo.AlbumImageUrl = ThumbnailUrl;
                videoInfo.YoutubeImageUrl = ThumbnailUrl;

                videoInfo.Path = Path.Combine(AppSettings.DownloadFolder, $"{sanatizedTitle} - {sanatizedAuthor}.mp3").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ");
